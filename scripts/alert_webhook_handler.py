...
class GitHubIssueManager:
    ...
    def update_github_issue(self, issue_number: int, alert_data: Dict[str, Any]) -> bool:
    ...
    def close_resolved_issue(self, issue_number: int, alert_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Failed to close issue: {response.status_code} - {response.text}")
                return False
    ...
    def _process_alert(self, alert: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if status.lower() == 'firing':
                ...
                if existing_issue:
                    success = self.github_manager.close_resolved_issue(existing_issue, alert_data)
                    if success:
                        return {
                            "status": "success",
                            "action": "closed_issue",
                    ...
                    return {"status": "info", "message": "No existing issue found to close"}
            
            return {"status": "info", "message": f"No action taken for status: {status}"}
