...
class HealthCheckMonitor:
        ...
        try:
            ...
            if is_healthy:
                ...
                uptime_result = subprocess.run(
                    ['systemctl', 'show', f'{self.service_name}.service', '--property=ActiveEnterTimestamp'],
                    capture_output=True,
                    text=True,
                    timeout=10
                )
    ...
    def check_resource_usage(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if issues:
                ...
                return True, f"Resources OK (CPU: {cpu_percent:.1f}%, Memory: {memory_percent:.1f}%, Disk: {disk_percent:.1f}%)"
                ...
                try:
                    ...
                    if response.status_code == 200:
                        if response_time > self.health_config['max_response_time_ms']:
                            results.append(f"{name}: Slow response ({response_time:.0f}ms)")
                            all_healthy = False
                        else:
    ...
    def perform_health_check(self) -> Dict:
        ...
        for check_name, check_func in checks:
            try:
                ...
                if not is_healthy:
                    all_healthy = False
                    self.logger.warning(f"Health check failed for {check_name}: {message}")
                else:
    ...
    def handle_unhealthy_service(self):
        ...
        if self.health_status['consecutive_failures'] >= self.health_config['restart_threshold']:
            self.logger.warning(f"Service unhealthy for {self.health_status['consecutive_failures']} consecutive checks, attempting restart...")
        ...
        try:
            while True:
                ...
                if health_status['overall'] != 'healthy':
                    ...
                    if self.health_status['consecutive_failures'] >= self.health_config['alert_threshold']:
                        self.send_alert(
                            'SERVICE_UNHEALTHY',
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Reddit Sentiment Pipeline Health Check")
    parser.add_argument('--once', action='store_true', help='Run health check once and exit')
    parser.add_argument('--continuous', action='store_true', help='Run continuous monitoring')
    parser.add_argument('--interval', type=int, default=300, help='Check interval in seconds (default: 300)')
    parser.add_argument('--config', type=str, default='/home/cayir/cicd_project/config', help='Configuration directory')
